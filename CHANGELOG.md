...
All notable changes to `rlm-notebook` are documented in this file. The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/), and the project follows [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

`rlm-notebook` is a research notebook built on [`rlm-harness`](https://github.com/qazbnm456/rlm-harness). You capture sources of any kind, ask questions grounded in them with citations you can check, and take a distilled artifact away.
...
#### Capture: the Inbox (Tier 0)

- The Inbox is the default screen. You paste a link or text, drop files, or upload them, and each capture lands at once as a node you can find again later. A notebook is a place you go into, and a node is filed into one or more notebooks by copying it (invariant 78).
- The Inbox is an index, not a corpus. Nodes live in a SQLite index plus one JSON file each, every write is a SQL delta, and nothing at Tier 0 assembles a blob, so the Inbox can hold thousands of items without touching the notebook size cap (invariants 8 and 78).
...
- Answers come back in the reader's language while citation coordinates stay exactly as the source has them. The interface language is a separate browser preference, and it counts as a signal when the notebook's output language is first chosen (invariants 39, 48 and 69).
...
- A notebook names itself from its sources, can be renamed at any time, and can be deleted (invariants 37 and 53).
...
- The overview is persisted on the notebook and marked stale when the sources change, instead of disappearing (invariant 38). Its "Start with" questions invite the first question and give way to the latest answer's "Ask next".
...
- Copy puts Markdown with its numbered references on the clipboard, from any answer, the overview or a guide. Export downloads the whole notebook as one Markdown file. Printing drops the application chrome, starts with the notebook's title and appends the reference list the printed numbers point to.
...
- `rlm-notebook serve` starts the API and the web UI on loopback. Every request needs the API token the server prints, and a non-loopback `--host` warns, because the token authenticates the app and there is still no per-user authorization (invariants 25 and 77).
...
- A run's state was repeatedly lost, duplicated or misplaced when the page repainted, a run failed, or the reader switched notebooks or reloaded. Now:
  - Each run owns its status row and its Stop.
  - A result lands only in its own notebook and its own tab.
...
- Deleting a notebook is refused while any work on it is in flight, including audio synthesis. A notebook that disappears mid-synthesis takes its audio with it, so a later notebook of the same name is never served someone else's episode.
...
- The SSRF guard is re-checked on every redirect hop, and operators behind a fake-IP resolver can carve out a range with `RN_FETCH_ALLOW_CIDRS` (invariants 2 and 76).
