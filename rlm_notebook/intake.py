"""The Inbox's intake queue: one worker, one item at a time.

`inbox.py` stores nodes; this is what fills them. The two are separate because parsing is
...
class IntakeQueue:
    """One worker thread draining a FIFO. Construct one per inbox directory; the API holds a
    ...
    def __init__(
        ...
        *,
        ...
        #: RESOLVED once, here, and never re-resolved. `inbox.inbox_dir` builds a `Path` from
        #: whatever it is handed, so a relative `"inbox"` would be resolved against the process's
    ...
    def submit(self, origin: str) -> Node:
        ...
        a URL already in the Inbox returns the node that is there rather than queueing it twice. A
        ...
        node = inbox.add_pending_node(origin, kind_for(origin), base_dir=self._base_dir)
        if node.state == "failed":
            ...
            node = inbox.update_node(node.id, base_dir=self._base_dir, state="queued", error=None) or node
    ...
    def base_dir(self) -> Path:
        """The RESOLVED inbox directory. Exposed because anything running on this queue's thread —
    ...
    def nudge(self) -> None:
        ...
        **Two capture paths never touch this queue**, and both are deliberate: pasted text and an
        uploaded file are already bytes in hand, so `capture_into_inbox` and the upload handler
    ...
    def resume_interrupted(self) -> list[str]:
        ...
        Rows in an OWNED state are reset first (`inbox.reset_interrupted_states`): the process that
        ...
        """
        # BOTH owned states, through the one map in `inbox.py`. `distilling` used to have no reset
        ...
        inbox.reset_interrupted_states(base_dir=self._base_dir)
        resumed = [
            node.id
            for node in inbox.list_nodes(state="queued", limit=10_000, base_dir=self._base_dir)
    ...
    def _maybe_idle(self) -> None:
        ...
        try:
            ...
            # raises `SystemExit` for a malformed `RN_*` value, which is a `BaseException` — so a
            # typo in `RN_AUTO_DISTIL_MAX_PER_BATCH` escaped an `except Exception` here, unwound
    ...
    def _process(self, node_id: str) -> None:
        ...
        """
        try:
            node = inbox.get_node(node_id, base_dir=self._base_dir)
            if node is None:
                # Removed while it sat in the queue. Not an error — the reader changed their mind.
                return
            inbox.update_node(node_id, base_dir=self._base_dir, state="parsing")
            ...
            inbox.store_blocks(node_id, source, base_dir=self._base_dir)
