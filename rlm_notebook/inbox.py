"""Tier 0 — the Inbox: everything captured, before any of it belongs to a notebook.
...
`docs/invariants/78-the-inbox-is-an-index-not-a-corpus.md` rather than restated here: **nothing at
...
## Storage
    ...
    inbox/index.db               SQLite index — one row per node, WAL
    inbox/nodes/<node_id>.json   the parsed Source's blocks, verbatim
...
directory, exactly like `notebooks/`, `traces/` and `audio/` — invariant 34's "where you START this
...
- **Import cost, since it is not obvious:** importing this reaches `rlm_harness` transitively —
  `notebook.py` -> `ingest.py` -> `parsers/web.py`, whose SSRF guard is `rlm_harness.tools.fetch`.
  It needs neither the `api` nor the `chatterbox` extra, and `rlm-harness` is a core dependency, so
  a bare `uv sync` is enough. Found by `test_inbox.py`'s subprocess blocker contradicting an earlier
...
- **WAL needs a real local filesystem** and degrades or fails outright on a network share. The same
  is already true of the notebook JSON files; it just fails more quietly there.
"""
...
#: Relative to the working directory, same as `notebook.DEFAULT_NOTEBOOKS_DIR` and for the same
#: reason (invariant 34).
...
#: `notebook.list_notebook_summaries` already applies on the way out (invariants 5/6: flag, never
...
CREATE TABLE IF NOT EXISTS memberships (
    ...
    PRIMARY KEY (node_id, notebook_id)
);
...
CREATE INDEX IF NOT EXISTS memberships_notebook ON memberships(notebook_id);
"""


def inbox_dir(base_dir: str | Path = DEFAULT_INBOX_DIR) -> Path:
    return Path(base_dir)


def index_path(base_dir: str | Path = DEFAULT_INBOX_DIR) -> Path:
    return inbox_dir(base_dir) / "index.db"
...
def node_blocks_path(node_id: str, *, base_dir: str | Path = DEFAULT_INBOX_DIR) -> Path:
    ...
    **Validated, because "the id is hex so it is filename-safe" is true of ids this module MINTS and
    ...
    `DELETE /inbox/{node_id}` that no guard was needed. It was not theoretical: `remove_node` then
    unlinked `f"{node_id}.json"` with no check, so `remove_node("../../notebooks/mynb")` deleted a
    live notebook file and returned `False`, and an absolute id discarded the directory entirely
    (`Path("inbox/nodes") / "/etc/x"` is `/etc/x`). Both reproduced before this was written.
    ...
    root = (inbox_dir(base_dir) / "nodes").resolve()
    path = (root / f"{node_id}.json").resolve()
    if not path.is_relative_to(root):
        raise ValueError(f"node id escapes the inbox: {node_id!r}")
    return path
...
def node_id_for(origin: str, text: str = "", *, origin_is_the_identity: bool | None = None) -> str:
    ...
    can be queued the id must be mintable with no content in hand. `capture_into_inbox` refuses
    ...
    **Every other origin folds in the TEXT, because a filename is not an identity.** Two different
    ...
    `notebook.append_sources`, from the max id in use (invariant 50).
    """
    stable = origin.strip()
...
def _initialize(path: Path) -> None:
    ...
    **`PRAGMA journal_mode=WAL` is the one statement here that a `busy_timeout` does not protect.**
    ...
    with _INIT_LOCK:
        ...
        # database was removed under a running process (`rm -rf inbox/` is an ordinary thing for a
...
@contextmanager
def _connect(base_dir: str | Path = DEFAULT_INBOX_DIR) -> Iterator[sqlite3.Connection]:
...
def add_node(
    ...
    *,
    ...
    base_dir: str | Path = DEFAULT_INBOX_DIR,
) -> Node:
    """Put an already-parsed `Source` into the Inbox, returning the node.
    ...
    **Idempotent on the node id, which is idempotent on the origin.** Capturing the same URL twice
    ...
    notebook's promotion already copied from.
...
def add_pending_node(
    origin: str, kind: str, *, base_dir: str | Path = DEFAULT_INBOX_DIR
) -> Node:
    ...
    **A node exists the moment it is enqueued, not when it finishes parsing.** `node_id_for` needs
    ...
    #: why a queued capture can be given a row before anything is fetched. `capture_into_inbox`
...
def get_node(node_id: str, *, base_dir: str | Path = DEFAULT_INBOX_DIR) -> Node | None:
    with _connect(base_dir) as conn:
        row = conn.execute("SELECT * FROM nodes WHERE id = ?", (node_id,)).fetchone()
    return _row_to_node(row) if row is not None else None


#: The columns a text query looks in, and the reason the Inbox needs no second store to be
...
#: 2.20ms (`SEARCH ... USING COVERING INDEX`). `GET /inbox` pays it twice and the page polls at
...
def list_nodes(
    *,
    ...
    past what a notebook could hold — a listing that loads all of it has the problem Tier 0 exists
    ...
    #: `list_notebook_summaries` uses for an unparseable notebook file.
    out: list[Node] = []
    for row in rows:
        try:
            out.append(_row_to_node(row))
        except (json.JSONDecodeError, ValidationError, TypeError) as exc:
            _log.warning("inbox: skipping unreadable row %r (%s)", dict(row).get("id"), exc)
    return out
...
def update_node(node_id: str, *, base_dir: str | Path = DEFAULT_INBOX_DIR, **fields: object) -> Node | None:
...
def reset_interrupted_states(*, base_dir: str | Path = DEFAULT_INBOX_DIR) -> dict[str, list[str]]:
...
def claim_node(
    node_id: str, *, expect: str, to: str, base_dir: str | Path = DEFAULT_INBOX_DIR
) -> bool:
...
def remove_node(node_id: str, *, base_dir: str | Path = DEFAULT_INBOX_DIR) -> bool:
    """Forget a node. Its memberships go with it (`ON DELETE CASCADE`), but **sources already
    promoted into notebooks STAY** — they were copied, and a notebook that silently lost a cited
    source because someone tidied their inbox would break invariant 12's promise that a source
...
def node_source(node_id: str, *, base_dir: str | Path = DEFAULT_INBOX_DIR) -> Source | None:
    ...
    handed against the receiving notebook (invariant 50), and `Source.marker()` derives the citation
...
class UnreadableNodeText(ValueError):
    ...
    **Raised where the cause is KNOWN, because the caller cannot tell.** `promote_node` reads the
    node's text and then writes a notebook, and `pydantic.ValidationError` comes out of both — so an
    API handler catching it broadly cannot say which file is broken. It tried, and told the operator
    to "fix or remove by hand" a `notebooks/<id>.json` that parsed perfectly well: destructive
...
def memberships_for(node_id: str, *, base_dir: str | Path = DEFAULT_INBOX_DIR) -> list[NodeMembership]:
    with _connect(base_dir) as conn:
        rows = conn.execute(
...
def forget_membership(
    notebook_id: str, source_id: str, *, base_dir: str | Path = DEFAULT_INBOX_DIR
) -> int:
    """Drop the membership rows that pointed at a source which no longer exists. Returns how many.

    **A membership outlived its source.** Promoting a node writes `(node_id, notebook_id,
    source_id)`; removing that source from the notebook (invariant 50 — the survivors are never
    renumbered, so the id is simply gone) left the row behind, so `GET /inbox/{node_id}` went on
    reporting the node as filed there and the Inbox row went on saying "already in <notebook>".
    ...
    the node: one node can be promoted into several notebooks, and only this notebook's copy went
    away.
    """
    with _connect(base_dir) as conn:
        cur = conn.execute(
            ...
            "DELETE FROM memberships WHERE notebook_id = ? AND source_id = ?",
            (slug(notebook_id), source_id),
        )
        return cur.rowcount or 0


def forget_notebook(notebook_id: str, *, base_dir: str | Path = DEFAULT_INBOX_DIR) -> int:
    """Drop every membership row for a notebook that no longer exists. Returns how many.

    The whole-notebook twin of `forget_membership`, for the same reason and with the same shape: an
    index entry pointing at something that is gone is simply wrong, and the one question a capture
    index has to answer is "where did this end up". Without it, deleting a notebook left every node
    it held reporting `In <that notebook>` forever — and the label would resolve, because the
    membership carries the slug and nothing re-checks that the file is still there.

    Nodes themselves are untouched: promotion COPIES (invariant 78), so a node outlives any notebook
    it was filed into, which is the same reasoning `remove_node` uses in the other direction.
    """
    with _connect(base_dir) as conn:
        cur = conn.execute("DELETE FROM memberships WHERE notebook_id = ?", (slug(notebook_id),))
        return cur.rowcount or 0


def nodes_in_notebook(notebook_id: str, *, base_dir: str | Path = DEFAULT_INBOX_DIR) -> list[NodeMembership]:
    with _connect(base_dir) as conn:
        rows = conn.execute(
            "SELECT * FROM memberships WHERE notebook_id = ? ORDER BY promoted_at ASC",
            (slug(notebook_id),),
        ).fetchall()
    return [NodeMembership.model_validate(dict(row)) for row in rows]


def _unused_origin(notebook, origin: str) -> str:
    ...
    """
    taken = {s.origin for s in notebook.sources}
    ...
    for n in range(2, 1000):
        ...
        if candidate not in taken:
            return candidate
    #: A thousand same-named files in one notebook is not a case worth a cleverer scheme; the
...
def promote_node(
    ...
    *,
    ...
    """Copy a node into a notebook as a real, citable `Source`, and record that it is there.

    Same verb and the same reasoning as `notebook.promote_note` (invariant 32 — a thing is uncited
    until promoted, and grounded exactly like any other source afterwards). Three properties that
    are each a decision:

    **The node is NOT consumed.** It stays in the Inbox and can be promoted into other notebooks.
    That is the "different facets of yourself" premise: the node is what persists, and a notebook is
    a view over a selection of them.

    **The id comes from `append_sources`, never from here.** It renumbers against the receiving
    notebook from the max id in use (invariant 50), inside `mutate_notebook`'s lock and against a
    re-read of the file (invariant 34) — so a concurrent write cannot make two sources share an id.

    **`create` says whether the CALLER meant to make a new notebook**, and defaults True only for
    the library's own callers. The HTTP layer passes what the reader actually chose: the picker in
    the Inbox is built from a notebook list fetched when the page rendered, so an option can name a
    notebook that has since been deleted — no race required. Promoting through that stale option
    re-created it, same handle, same slug, holding one source and none of its title, sources, notes,
    turns or overview, and it reappeared in the facet rail. `DELETE /notebooks/{id}` refuses while a
    ...
    A notebook that does not exist yet is created when `create` is True. Promoting into a new name is
    how a reader starts a facet, so refusing would make "file this somewhere new" a two-step. The
    cost is that a typo'd notebook id silently creates a notebook rather than 404ing; a caller that
    needs the strict behaviour should resolve the notebook first.

    **Promoting something the notebook already has is not an error.** `append_sources` dedupes by
    origin and returns only what it actually appended; when it appends nothing, the notebook already
    ...
    #: snapshot taken before `mutate_notebook`, and invariant 34 names that fault exactly: a stale
    #: snapshot and interleaved critical sections are two distinct problems, and the lock alone does
    #: not fix the first. Measured with ten concurrent promotions of one node into one notebook:
    #: **8 of 10 answered 400** saying the notebook "already holds a DIFFERENT source with origin
    ...
    def prior_membership():
        return next(
            (
                ...
                if m.notebook_id == slug(notebook_id)
            ),
            None,
        )
    ...
    def apply(notebook):
        appended = append_sources(notebook, [source])
        if appended:
        ...
        # notebook, and `promote_node` returned success. Reproduced before this was written.
        ...
        if prior is not None and any(s.id == prior.source_id for s in notebook.sources):
            assigned.append(prior.source_id)
            return
        other = next((s for s in notebook.sources if s.origin == source.origin), None)
        if other is None:
            return
        #: **Is that source this node's CONTENT, or somebody else's?** The membership table is the
        #: obvious place to ask and it is racy: the row is written after `mutate_notebook` releases
        ...
        renamed = source.model_copy(update={"origin": _unused_origin(notebook, source.origin)})
        again = append_sources(notebook, [renamed])
        if again:
    ...
    # Whether the notebook EXISTED before this call, so the rollback below knows whether the file
    # itself is ours to undo. `create=True` means a promotion that fails afterwards can otherwise
    # leave a notebook nobody asked for: twelve concurrent promote/delete races all correctly lost
    # the promote and all twelve left `racebook.json` behind, showing in the facet rail as
    # "Untitled notebook · 0".
    existed = notebook_path(notebook_id, base_dir=notebooks_dir).exists()
    mutate_notebook(notebook_id, apply, base_dir=notebooks_dir, create=create)
    if conflict and not assigned:
        raise ValueError(
            f"{notebook_id!r} already holds a DIFFERENT source ({conflict[0]}) with origin "
            f"{source.origin!r}, so node {node_id!r} cannot be added without shadowing it"
        )
    if not assigned:
        raise ValueError(f"node {node_id!r} was neither appended to nor found in {notebook_id!r}")

    membership = NodeMembership(
        # **KEYED ON THE SLUG, because the slug is what identifies the notebook FILE.** This stored
        # the raw id while `notebook.slug()` decides the filename, so `"Foo Bar"` and `"Foo-Bar"`
        # were one notebook on disk and two rows here: removing the source through the file's id
        ...
        node_id=node_id,
        notebook_id=slug(notebook_id),
        source_id=assigned[0],
        promoted_at=time.time(),
    )
    try:
        with _connect(base_dir) as conn:
            # `INSERT OR REPLACE`, because promoting the same node into the same notebook twice is
            # the same "not an error" case as above — the second one just restates where it went.
            conn.execute(
                "INSERT OR REPLACE INTO memberships (node_id, notebook_id, source_id, promoted_at) "
                "VALUES (?, ?, ?, ?)",
                (membership.node_id, membership.notebook_id, membership.source_id, membership.promoted_at),
            )
    except sqlite3.IntegrityError as exc:
        # **The node was REMOVED while this promotion was running.** `memberships.node_id` has a
        # foreign key, so the insert is refused — and by then `mutate_notebook` has already written
        # the source into the notebook file. Measured before this branch existed: two of twelve
        # concurrent promote/delete pairs returned a raw 500 AND left the notebook holding a source
        # no membership row records. The caller saw a failure; the notebook silently grew.
        #
        ...
        # And the notebook FILE, when this call is what created it and the undo left it empty.
        # Anything else is someone else's notebook: a pre-existing one is never removed, and one
        # that gained content from another writer in the meantime is not ours to judge.
        if not existed:
            path = notebook_path(notebook_id, base_dir=notebooks_dir)
            with contextlib.suppress(OSError, ValueError):
                fresh = load_notebook(notebook_id, base_dir=notebooks_dir)
                if fresh is not None and not fresh.sources:
                    path.unlink(missing_ok=True)
        raise ValueError(
            f"node {node_id!r} was removed while it was being promoted into {notebook_id!r}; "
            "nothing was added"
        ) from exc
    return membership
