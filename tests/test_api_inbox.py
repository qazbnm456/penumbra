"""The Inbox's HTTP surface (`/inbox/*`).
...
from rlm_notebook import api, auth, inbox, ingest, intake
...
def _a_model_is_configured(monkeypatch):
    ...
    `RN_*` set even though every one of them replaces `distil_source` with a stand-in.

    That is not fixture noise, it is the defect this file could not see. `config.setup()` was never
    called anywhere in the server process, so `POST /inbox/distil` could only ever produce
    ...
    """
    monkeypatch.setenv("RN_MAIN_MODEL", "openai/gpt-4o-mini")
    monkeypatch.setenv("RN_API_KEY", "sk-test")
...
def _settle(client, node_id: str, *, want: str = "ready_undistilled", tries: int = 100) -> str:
    ...
    for _ in range(tries):
        state = client.get(f"/inbox/{node_id}").json()["node"]["state"]
        if state == want:
            return state
        time.sleep(0.02)
    return state
...
def test_listing_the_inbox_reports_a_clean_500_when_the_corpus_cap_env_var_is_malformed(
    client, monkeypatch
):
    ...
    """
    monkeypatch.setenv("RN_MAX_CORPUS_CHARS", "8M")
    ...
    monkeypatch.delenv("RN_MAX_CORPUS_CHARS")
...
def test_a_malformed_corpus_cap_refuses_STARTUP_rather_than_waiting_for_the_first_request(
    ...
    Drives `_lifespan` directly, for the reason the `RN_AUTO_DISTIL_MAX_PER_BATCH` test beside it
    ...
    monkeypatch.setenv("RN_MAX_CORPUS_CHARS", "nope")

    async def enter_and_leave() -> None:
        async with api._lifespan(api.app):
            pass

    with pytest.raises(SystemExit, match="RN_MAX_CORPUS_CHARS"):
...
def test_a_local_path_is_refused_and_never_read(client, tmp_path):
    ...
    than inherited from the notebook one.
    """
    ...
    for hostile in (str(secret), "/etc/passwd", "file:///etc/passwd", "../../etc/passwd", "~/.ssh/id_rsa"):
        resp = client.post("/inbox", json={"urls": [hostile]})
        assert resp.status_code == 422, hostile
        assert "invariant 26" in resp.json()["detail"]
        assert "SUPER-SECRET" not in resp.text

    assert client.get("/inbox").json()["total"] == 0, "a refused capture must create nothing"
...
def test_a_url_capture_returns_the_node_immediately_as_queued(client):
    ...
    body = client.post("/inbox", json={"urls": ["https://example.com/a"]}).json()
    node = body["nodes"][0]
    assert node["state"] == "queued"
    assert node["kind"] == "web"
    assert _settle(client, node["id"]) == "ready_undistilled"
    stored = client.get(f"/inbox/{node['id']}/source").json()["source"]
...
def test_pasted_text_skips_the_queue_and_is_ready_at_once(client):
    ...
    node = client.post("/inbox", json={"texts": ["a pasted note"]}).json()["nodes"][0]
    ...
    again = client.post("/inbox", json={"texts": ["a pasted note"]}).json()["nodes"][0]
    assert again["id"] == node["id"]
    assert client.get("/inbox").json()["total"] == 1


def test_an_empty_capture_is_a_client_error(client):
    assert client.post("/inbox", json={}).status_code == 422
    assert client.post("/inbox", json={"texts": ["   "]}).json()["nodes"] == []


def test_an_unknown_field_is_refused_rather_than_ignored(client):
    assert client.post("/inbox", json={"sources": ["https://example.com/a"]}).status_code == 422
...
def test_the_listing_is_paged_and_reports_what_a_summary_pass_would_cost(client):
    for n in range(5):
        client.post("/inbox", json={"texts": [f"note {n}"]})
    page = client.get("/inbox?limit=2").json()
    assert len(page["nodes"]) == 2
    assert page["total"] == 5
    # Invariant 80: the count is knowable BEFORE anyone spends anything.
    assert page["undistilled"] == 5
    assert client.get("/inbox?limit=2&offset=4").json()["nodes"][0]["id"] not in {
...
def test_a_malformed_id_is_400_and_a_missing_one_is_404(client):
    ...
    `inbox.node_blocks_path` raises `ValueError` for anything that is not a minted id — the same
    guard that stops `remove_node("../../notebooks/mynb")` deleting a live notebook file."""
    for endpoint in ("/inbox/{}", "/inbox/{}/source"):
    ...
    assert client.delete("/inbox/nd-not-hex").status_code == 400
    assert client.post(
        "/inbox/nd-not-hex/promote",
        json={"notebook_id": "x", "create": True},
    ).status_code == 400


def test_deleting_a_node_leaves_a_promoted_source_alone(client):
    node = client.post("/inbox", json={"texts": ["keep me"]}).json()["nodes"][0]
    client.post(f"/inbox/{node['id']}/promote", json={"notebook_id": "mynb", "create": True})

    assert client.delete(f"/inbox/{node['id']}").json()["removed"] is True
    assert client.get(f"/inbox/{node['id']}").status_code == 404
    # Invariant 12: promotion is a COPY, and tidying an inbox must not reach a cited source.
    assert len(client.get("/notebooks/mynb").json()["sources"]) == 1


# --- promotion -------------------------------------------------------------------------------------


def test_promotion_assigns_the_notebooks_own_source_id_and_keeps_the_node(client):
    node = client.post("/inbox", json={"texts": ["promote me"]}).json()["nodes"][0]
    membership = client.post(
        f"/inbox/{node['id']}/promote",
        json={"notebook_id": "work", "create": True},
    ).json()

    assert membership["membership"]["source_id"] != node["id"]
    assert client.get(f"/inbox/{node['id']}").json()["node"] is not None
    # The node is not consumed, so the same one can go into a second facet.
    client.post(f"/inbox/{node['id']}/promote", json={"notebook_id": "personal", "create": True})
    listed = client.get(f"/inbox/{node['id']}").json()["notebooks"]
    assert {m["notebook_id"] for m in listed} == {"work", "personal"}
...
def test_promoting_a_node_with_no_text_yet_is_a_clean_400(client):
    ...
    `ValueError` BEFORE `mutate_notebook` — no empty notebook is created as a side effect."""
    node = inbox.add_pending_node("https://example.com/waiting", "web")
    resp = client.post(f"/inbox/{node.id}/promote", json={"notebook_id": "nb", "create": True})
    assert resp.status_code == 400
    assert client.get("/notebooks/nb").status_code == 404
...
def test_status_and_cancel_report_what_is_actually_happening(client):
    ...
    assert client.get("/inbox/status").json() == {
        "running": False,
        "current": None,
        "pending": 0,
        "distil": {"running": False, "done": 0, "total": 0, "failed": 0, "error": ""},
    }
    body = client.post("/inbox/cancel").json()
...
def test_capturing_makes_no_model_call(client, monkeypatch):
    ...
    from rlm_notebook import distill

    calls: list[object] = []
    monkeypatch.setattr(distill, "distil_source", lambda *a, **k: calls.append(a) or Distillation())

    for n in range(3):
        client.post("/inbox", json={"urls": [f"https://example.com/{n}"]})
    for node in client.get("/inbox").json()["nodes"]:
        _settle(client, node["id"])
    assert calls == [], "capture spent the reader's money"
    assert client.get("/inbox").json()["undistilled"] == 3
...
def test_distil_is_a_separate_verb_that_names_its_own_number(client, monkeypatch):
    ...
    assert client.post("/inbox/distil", json={"limit": 0}).status_code == 422
    ...
    # it is about to spend and `GET /inbox/status` carries the rest.
    started = client.post("/inbox/distil", json={"limit": 2}).json()
    assert started == {
        "started": True,
        "running": True,
        "done": 0,
    ...
    for _ in range(200):
        if client.get("/inbox/status").json()["distil"]["running"] is False:
            break
        time.sleep(0.02)
    assert client.get("/inbox/status").json()["distil"]["running"] is False
    assert client.get("/inbox").json()["undistilled"] == 1
    assert client.get("/inbox?state=ready").json()["nodes"][0]["title"] == "T"


def test_auto_distillation_is_off_by_default_and_on_when_the_operator_says_so(client, monkeypatch):
    """The toggle the settings page exposes (a BEHAVIOUR preference — `POST /inbox/distil` is
    already a spend endpoint any token holder can call). Its BOUND,
    `RN_AUTO_DISTIL_MAX_PER_BATCH`, stays environment-only, which is invariant 41's placement rule.
    """
    from rlm_notebook import distill
    ...
    monkeypatch.delenv("RN_AUTO_DISTIL", raising=False)
    node = client.post("/inbox", json={"urls": ["https://example.com/off"]}).json()["nodes"][0]
    _settle(client, node["id"])
    assert seen == [], "the default must not spend"

    monkeypatch.setenv("RN_AUTO_DISTIL", "on")
    node = client.post("/inbox", json={"urls": ["https://example.com/on"]}).json()["nodes"][0]
    for _ in range(100):
        if client.get(f"/inbox/{node['id']}").json()["node"]["state"] == "ready":
            break
        time.sleep(0.02)
    assert seen, "the idle hook never fired"
    assert client.get(f"/inbox/{node['id']}").json()["node"]["title"] == "auto"
...
def test_the_auto_pass_never_reports_more_done_than_it_said_it_would(client, monkeypatch):
    ...
    makes the race deterministic instead of a sleep — and it goes in through `inbox.add_node`
    ...
    """
    from rlm_notebook import api, distill, inbox
    from rlm_notebook.schema import Source, SourceBlock

    monkeypatch.setenv("RN_AUTO_DISTIL", "on")
    monkeypatch.setenv("RN_AUTO_DISTIL_MAX_PER_BATCH", "5")
    ...
    node = client.post("/inbox", json={"texts": ["the first note"]}).json()["nodes"][0]
    _settle(client, node["id"])
    for _ in range(200):
        if not client.get("/inbox/status").json()["distil"]["running"] and calls:
            break
        time.sleep(0.02)
...
def test_two_uploaded_files_with_the_same_name_are_two_nodes(client):
    ...
    texts = {
        "".join(b["text"] for b in client.get(f"/inbox/{node_id}/source").json()["source"]["blocks"])
        for node_id in ids
    }
    ...
    again = client.post(
        "/inbox/upload", files=[("file", ("notes.txt", b"the first file's contents", "text/plain"))]
    )
    assert again.json()["nodes"][0]["id"] == ids[0]
    assert client.get("/inbox").json()["total"] == 2
...
def test_the_same_url_captured_twice_is_still_one_node_even_if_its_text_changed(client, monkeypatch):
    ...
    first = inbox.node_id_for("https://example.com/a", "what the page said in March")
    second = inbox.node_id_for("https://example.com/a", "what it says now")
    pending = inbox.node_id_for("https://example.com/a")
    assert first == second == pending, "a URL's identity is the URL"


def test_the_notebook_upload_refuses_a_batch_instead_of_keeping_the_last_file(client):
    ...
    with the one that would have worked neither stored nor mentioned. `/inbox/upload` is the batch
    ...
    """
    resp = client.post(
        "/notebooks/reading/sources/upload",
        files=[
    ...
    assert client.get("/notebooks/reading").status_code == 404
...
def test_the_upload_cap_is_enforced_from_content_length(client, monkeypatch):
    ...
    resp = client.post(
        "/inbox/upload", files={"file": ("big.txt", b"x" * 100, "text/plain")}
    )
    assert resp.status_code == 413
    assert client.get("/inbox").json()["total"] == 0


def test_an_upload_lands_as_a_node(client):
    resp = client.post("/inbox/upload", files={"file": ("notes.md", b"# heading", "text/markdown")})
...
def test_an_unsupported_upload_type_is_refused_by_name(client):
    resp = client.post("/inbox/upload", files={"file": ("sheet.xlsx", b"PK\x03\x04", "application/zip")})
    assert resp.status_code == 422
...
def test_every_inbox_route_needs_the_token():
    ...
    assert anon.get("/inbox").status_code == 401
    assert anon.post("/inbox", json={"texts": ["x"]}).status_code == 401
    assert anon.get("/inbox/status").status_code == 401
    assert anon.post("/inbox/distil", json={"limit": 1}).status_code == 401
    assert anon.delete("/inbox/nd-" + "0" * 16).status_code == 401
...
def test_a_second_server_lifecycle_can_still_capture(_fake_web):
    ...
    for cycle in range(2):
        with TestClient(api.app, base_url="http://127.0.0.1", headers=headers) as c:
            node = c.post("/inbox", json={"urls": [f"https://example.com/cycle{cycle}"]}).json()["nodes"][0]
...
def test_the_listing_cap_cannot_be_defeated_by_a_negative_limit(client):
    ...
    assert len(client.get("/inbox?limit=200").json()["nodes"]) == 200
    ...
    assert client.get("/inbox?limit=500").status_code == 422
    assert client.get("/inbox?limit=-1").status_code == 422
    assert client.get("/inbox?limit=0").status_code == 422
...
def test_out_of_range_integers_are_a_422_not_a_500(client):
    ...
    assert client.get("/inbox?offset=100000000000000000000").status_code == 422
    assert client.post("/inbox/distil", json={"limit": 100000000000000000000}).status_code == 422
    assert client.post("/inbox/distil", json={"limit": 0}).status_code == 422


def test_promotion_losing_a_race_with_deletion_leaves_the_notebook_unchanged(client):
    """**The hard stop.** `memberships.node_id` has a foreign key, so a node removed mid-promotion
    made the INSERT fail — by which time `mutate_notebook` had already written the source. Measured:
    two of twelve concurrent pairs returned a raw 500 AND left the notebook holding a source no
    membership row recorded. The caller saw a failure; the notebook silently grew.
    ...
    node = client.post("/inbox", json={"texts": ["racy"]}).json()["nodes"][0]
    real_append = inbox.append_sources

    def append_then_vanish(notebook, sources):
        appended = real_append(notebook, sources)
        inbox.remove_node(node["id"])          # the DELETE lands here
        return appended

    inbox.append_sources = append_then_vanish
    try:
        resp = client.post(f"/inbox/{node['id']}/promote", json={"notebook_id": "racenb", "create": True})
    finally:
        inbox.append_sources = real_append

    assert resp.status_code == 400, resp.text
    assert "removed while it was being promoted" in resp.json()["detail"]
    book = load_notebook("racenb")
...
def test_every_uploaded_file_is_ingested_not_just_the_last(client):
    ...
    assert client.get("/inbox").json()["total"] == 3
...
def test_a_malformed_multipart_body_is_a_400_not_a_raw_500(client):
    ...
    as plain-text `Internal Server Error`. Pre-existing on the notebook uploader, which the Inbox's
    copied — so both are fixed, and both are asserted."""
    for endpoint in ("/inbox/upload", "/notebooks/mynb/sources/upload"):
        resp = client.post(
            endpoint,
...
def test_the_idle_hook_yields_to_a_capture_that_arrives_mid_batch(client, monkeypatch):
    ...
    for n in range(6):
        client.post("/inbox", json={"texts": [f"note {n}"]})
    client.post("/inbox", json={"urls": ["https://example.com/trigger"]})

    assert reached.wait(5), "the idle hook never started summarising"
    client.post("/inbox", json={"urls": ["https://example.com/urgent"]})
    held.set()
...
def test_asking_for_a_summary_pass_with_nothing_pending_answers_instead_of_wedging(
    ...
    `distil_inbox` holds `_DISTIL_GUARD` — a plain, non-reentrant `threading.Lock` — and its
    ...
    Every existing `/inbox/distil` test posts with pending nodes, so `total > 0` in all of them. The
    ...
    assert client.get("/inbox/status").json()["distil"]["running"] is False
    resp = client.post("/inbox/distil", json={"limit": 4})
    assert resp.status_code == 200, resp.text
    ...
    assert client.get("/inbox/status").status_code == 200
    assert client.get("/inbox").status_code == 200
...
def test_no_handler_re_enters_the_summary_lock(client, monkeypatch):
    ...
    assert client.post("/inbox/distil", json={"limit": 4}).status_code == 200
    assert client.post("/inbox/cancel").status_code == 200
    assert client.get("/inbox/status").status_code == 200
...
def test_a_failed_summary_pass_can_be_dismissed_instead_of_greeting_everyone_forever(client):
    ...
    """
    api._DISTIL.update({"running": False, "failed": 2, "error": "RuntimeError: RN_MAIN_MODEL is not set"})
    assert client.get("/inbox/status").json()["distil"]["error"], "the fixture did not take"

    resp = client.post("/inbox/distil/dismiss")
    assert resp.status_code == 200, resp.text
    ...
    after = client.get("/inbox/status").json()["distil"]
...
def test_a_running_summary_pass_owns_its_error_fields(client):
    ...
    try:
        assert client.post("/inbox/distil/dismiss").status_code == 409
        assert client.get("/inbox/status").json()["distil"]["error"] == "one node failed"
    finally:
...
def test_a_malformed_auto_distil_bound_refuses_startup(monkeypatch):
    ...
    monkeypatch.setenv("RN_AUTO_DISTIL_MAX_PER_BATCH", "not-an-int")

    async def enter_and_leave() -> None:
        async with api._lifespan(api.app):
            pass

    with pytest.raises(SystemExit, match="RN_AUTO_DISTIL_MAX_PER_BATCH"):
        asyncio.run(enter_and_leave())
...
def test_the_idle_hook_can_never_end_the_worker(client, monkeypatch):
    ...
    def explode(*args, **kwargs):
        raise SystemExit("a malformed RN_* value, as config raises it")

    monkeypatch.setattr(distill, "distil_pending", explode)
    first = client.post("/inbox", json={"urls": ["https://example.com/a"]}).json()["nodes"][0]
    _settle(client, first["id"])

    second = client.post("/inbox", json={"urls": ["https://example.com/b"]}).json()["nodes"][0]
...
def test_a_batch_upload_keeps_every_file_it_can_and_names_the_rest(client):
    ...
    that survived nor the one that vanished. An independent review reproduced it: the inbox total
    ...
    only_bad = client.post("/inbox/upload", files=[("file", ("x.docx", b"nope", "application/octet-stream"))])
...
def test_the_listing_reports_the_ceiling_a_node_has_to_fit_under(client, monkeypatch):
    ...
    text file captures fine, promotes fine, and then makes the notebook it was promoted into
    ...
    Read through `config.max_corpus_chars()`, which is independent of `NotebookConfig` for the same
    ...
    monkeypatch.setenv("RN_MAX_CORPUS_CHARS", "1234")
    assert client.get("/inbox").json()["corpus_char_cap"] == 1234, "the env has to win, as everywhere"
...
def test_the_listing_actually_filters_by_the_query(client):
    ...
    `inbox._search_clause` has six good unit tests and `?state=` / `?offset=` are driven over HTTP,
    ...
    for text in ("a note about gardens", "a note about rivers", "a note about both gardens and rivers"):
        client.post("/inbox", json={"texts": [text]})

    def origins(**params):
        reply = client.get("/inbox", params=params)
        assert reply.status_code == 200, reply.text
    ...
    assert client.get("/inbox", params={"q": "gardens"}).json()["total"] == 2
    assert client.get("/inbox", params={"q": "zzzznothing"}).json()["nodes"] == []
...
def test_cancel_actually_reaches_a_running_summary_pass(client, monkeypatch):
    ...
    `test_status_and_cancel_report_what_is_actually_happening` posts to `/inbox/cancel` and then
    ...
    started = client.post("/inbox/distil", json={"limit": 12}).json()
    ...
    client.post("/inbox/cancel")

    for _ in range(200):
        if not client.get("/inbox/status").json()["distil"]["running"]:
            break
        time.sleep(0.02)
    ...
    assert client.get("/inbox/status").json()["distil"]["running"] is False
    assert client.get("/inbox").json()["undistilled"] == 12 - stopped_after
...
def test_a_second_summary_pass_is_refused_while_one_is_running(client, monkeypatch):
    ...
    for n in range(8):
        client.post("/inbox", json={"texts": [f"slow note {n}"]})

    assert client.post("/inbox/distil", json={"limit": 8}).json()["started"] is True
    second = client.post("/inbox/distil", json={"limit": 8})
...
def test_a_failed_node_is_counted_and_its_reason_reported(client, monkeypatch):
    ...
    for n in range(2):
        client.post("/inbox", json={"texts": [f"doomed {n}"]})

    client.post("/inbox/distil", json={"limit": 2})
    for _ in range(200):
        distil = client.get("/inbox/status").json()["distil"]
...
def test_a_malformed_file_is_refused_not_a_500_and_takes_nothing_with_it(client):
    ...
    only_bad = client.post(
        "/inbox/upload", files=[("file", ("broken.pdf", b"%PDF-1.7 rubbish", "application/pdf"))]
    )
    assert only_bad.status_code == 422, only_bad.text


def test_the_notebook_upload_refuses_a_malformed_file_too(client):
...
def test_a_membership_does_not_outlive_the_source_it_points_at(client):
    ...
    Promotion writes `(node_id, notebook_id, source_id)`. Removing that source from the notebook
    left the row behind — invariant 50 guarantees the survivors are never renumbered, so the id is
    simply gone — and `GET /inbox/{node_id}` went on reporting the node as filed there while the
    Inbox row went on saying "already in <notebook>". Re-promoting worked, so nothing was lost;
    the index was just wrong about the only thing it is for.
    """
    node = client.post("/inbox", json={"texts": ["something worth filing"]}).json()["nodes"][0]
    _settle(client, node["id"])

    promoted = client.post(f"/inbox/{node['id']}/promote", json={"notebook_id": "reading", "create": True})
    assert promoted.status_code == 200
    filed = client.get(f"/inbox/{node['id']}").json()["notebooks"]
    assert [(m["notebook_id"], m["source_id"]) for m in filed] == [("reading", "s1")]

    assert client.delete("/notebooks/reading/sources/s1").status_code == 200
    assert client.get(f"/inbox/{node['id']}").json()["notebooks"] == [], (
        "the node still claims to be filed in a notebook whose copy of it has been removed"
    )

    # And it can be filed again, which is what makes the stale row a lie rather than a record.
    again = client.post(f"/inbox/{node['id']}/promote", json={"notebook_id": "reading", "create": True})
    assert again.status_code == 200
    assert [m["notebook_id"] for m in client.get(f"/inbox/{node['id']}").json()["notebooks"]] == [
        "reading"
    ]
...
def test_an_unreadable_blocks_file_is_a_409_naming_the_right_file(client, tmp_path):
    ...
    `GET /inbox/{id}/source` let the exception escape as a bodyless `500 Internal Server Error`, so
    ...
    `POST /inbox/{id}/promote` was worse: it caught `ValidationError` broadly and blamed the
    NOTEBOOK, telling the operator to "fix or remove by hand" a `notebooks/<id>.json` that parses
    perfectly well. `promote_node` reads the node first and the notebook second, and both raise the
    same class — so the narrowing belongs at the call that knows which file it was reading.
    """
    from rlm_notebook import inbox

    node = client.post("/inbox", json={"texts": ["something worth keeping"]}).json()["nodes"][0]
    _settle(client, node["id"])
    assert client.get(f"/inbox/{node['id']}/source").status_code == 200

    inbox.node_blocks_path(node["id"]).write_text('{"not": "a list of blocks"}', encoding="utf-8")

    read = client.get(f"/inbox/{node['id']}/source")
    ...
    assert "notebooks/" not in detail, (
        f"a broken NODE file was blamed on a notebook that parses fine, with advice to remove it: "
        f"{detail}"
    )


def test_a_membership_is_keyed_on_the_same_thing_the_notebook_file_is(client):
    """**One notebook on disk, two rows in the index, and the second promotion reused a source id.**

    `notebook.slug()` decides the FILENAME, so `"Foo Bar"` and `"Foo-Bar"` are one notebook — but
    ...
    """
    first = client.post("/inbox", json={"texts": ["the first thing"]}).json()["nodes"][0]
    second = client.post("/inbox", json={"texts": ["the second thing"]}).json()["nodes"][0]
    _settle(client, first["id"])
    _settle(client, second["id"])

    assert client.post(
        f"/inbox/{first['id']}/promote",
        json={"notebook_id": "Foo Bar", "create": True},
    ).status_code == 200
    filed = client.get(f"/inbox/{first['id']}").json()["notebooks"]
    assert [m["notebook_id"] for m in filed] == ["Foo-Bar"], (
        f"the membership does not name the notebook the file is: {filed}"
    )

    # The SAME notebook, named the way its file is. Removing through it must clear the row.
    assert client.delete("/notebooks/Foo-Bar/sources/s1").status_code == 200
    assert client.get(f"/inbox/{first['id']}").json()["notebooks"] == [], (
    ...
    assert client.post(
        f"/inbox/{second['id']}/promote",
        json={"notebook_id": "Foo-Bar", "create": True},
    ).status_code == 200
    assert [m["notebook_id"] for m in client.get(f"/inbox/{second['id']}").json()["notebooks"]] == ["Foo-Bar"]
    assert client.get(f"/inbox/{first['id']}").json()["notebooks"] == [], (
...
def test_one_unreadable_row_does_not_take_down_the_whole_listing(client):
    """**`GET /inbox` is the application's front page**, and a single unparseable row turned it into
    ...
    `list_notebook_summaries` already uses for an unparseable notebook file.
    """
    import sqlite3

    from rlm_notebook import inbox

    good = client.post("/inbox", json={"texts": ["a node that is fine"]}).json()["nodes"][0]
    bad = client.post("/inbox", json={"texts": ["a node about to be corrupted"]}).json()["nodes"][0]
...
def test_an_unreadable_index_row_can_be_removed_and_stops_inflating_the_counts(client):
    ...
    `list_nodes` skipped it so `GET /inbox` would stop 500ing — and `count_nodes` was a separate
    ...
    read = client.get(f"/inbox/{bad['id']}")
    ...
    # immortal.
    gone = client.delete(f"/inbox/{bad['id']}")
    assert gone.status_code == 200 and gone.json()["removed"] is True
    assert client.get(f"/inbox/{bad['id']}").status_code == 404
    assert client.get("/inbox").json()["total"] == 1

    # A malformed id is still a 400, and an absent one still a 404 - invariant 27's distinction.
    assert client.delete("/inbox/not-a-minted-node-id").status_code == 400
    assert client.delete("/inbox/nd-0000000000000000").status_code == 404
...
def test_concurrent_promotions_of_one_node_do_not_blame_each_other(client):
    ...
    `promote_node` read this node's prior membership BEFORE `mutate_notebook` took the lock. Under
    ...
    `None`, and it falls through to "the notebook already holds a DIFFERENT source with this
    ...
    node = client.post("/inbox", json={"texts": ["one node, many eager promoters"]}).json()["nodes"][0]
    _settle(client, node["id"])

    def promote(_: int):
        return client.post(f"/inbox/{node['id']}/promote", json={"notebook_id": "racey", "create": True})
    ...
    assert set(codes) == {200}, (
        f"concurrent promotions of ONE node into ONE notebook must all be idempotent, got {codes}: "
    ...
    assert len(client.get("/notebooks/racey").json()["sources"]) == 1
    assert len(client.get(f"/inbox/{node['id']}").json()["notebooks"]) == 1
...
def test_a_source_another_route_added_under_the_same_origin_is_not_shadowed(tmp_path):
    ...
    so the node's own text never reached the notebook and `promote_node` returned success.
    ...
    is between a node and a source the notebook got by another route: `cli.py --source`, or
    `POST /notebooks/{id}/sources`, on a URL whose content has changed since.
    ...
    """
    from rlm_notebook import inbox
    from rlm_notebook.notebook import load_notebook, mutate_notebook
    from rlm_notebook.schema import Source, SourceBlock

    base = tmp_path / "inbox"
    books = tmp_path / "notebooks"
    node = inbox.add_node(
        Source(
            id="s0",
            kind="web",
    ...
    # The notebook already holds that origin, with DIFFERENT text, from another route.
    mutate_notebook(
        "collide",
    ...
    membership = inbox.promote_node(node.id, "collide", base_dir=base, notebooks_dir=books)
    assert membership.source_id != "s1", (
        "the membership points at a source that is not this node's text — the data-loss bug"
    )
    sources = {s.id: s for s in load_notebook("collide", base_dir=books).sources}
...
def test_re_promoting_a_node_whose_text_changed_is_still_idempotent(client):
    ...
    node = client.post("/inbox", json={"texts": ["the original text"]}).json()["nodes"][0]
    _settle(client, node["id"])
    promoted = client.post(f"/inbox/{node['id']}/promote", json={"notebook_id": "changed", "create": True})
    assert promoted.status_code == 200
    ...
    again = client.post(f"/inbox/{node['id']}/promote", json={"notebook_id": "changed", "create": True})
    assert again.status_code == 200, (
    ...
    assert len(client.get("/notebooks/changed").json()["sources"]) == 1
...
def test_a_promotion_racing_its_own_sibling_adopts_the_source_instead_of_refusing(tmp_path):
    ...
    `promote_node` writes the notebook under a `flock` and the membership row into SQLite
    ...
    first = inbox.promote_node(node.id, "racing", base_dir=base, notebooks_dir=books)

    # The source is in the notebook; the row that says whose it is has not landed yet.
    inbox.forget_membership("racing", first.source_id, base_dir=base)

    second = inbox.promote_node(node.id, "racing", base_dir=base, notebooks_dir=books)
    assert second.source_id == first.source_id, (
        "a promotion racing its own sibling was refused, or filed the node twice"
    )
    assert len(inbox.nodes_in_notebook("racing", base_dir=base)) == 1
...
def test_two_files_with_the_same_name_can_both_be_filed(client):
    ...
    the second into a notebook that already held the first was then a hard refusal with no way out,
    ...
    is an ordinary thing to drop on an inbox.
    """
    resp = client.post(
        "/inbox/upload",
        files=[
    ...
    a = client.post(f"/inbox/{first}/promote", json={"notebook_id": "both", "create": True})
    b = client.post(f"/inbox/{second}/promote", json={"notebook_id": "both", "create": True})
    assert a.status_code == 200 and b.status_code == 200, (a.text, b.text)

    sources = client.get("/notebooks/both").json()["sources"]
    ...
    texts = {
        s["id"]: "".join(
            b["text"]
            for b in client.get(f"/notebooks/both/sources/{s['id']}").json()["blocks"]
        )
        for s in sources
    }
...
def test_the_notebook_listing_carries_the_key_a_membership_is_written_with(client):
    """**The UI reported a live notebook as deleted.**

    `promote_node` keys `NodeMembership.notebook_id` on `slug(notebook_id)` — deliberately, because
    the slug is what identifies the notebook FILE. The client built its label map from
    `GET /notebooks`, which carried the id stored INSIDE the file and no slug, so for any notebook
    whose id differs from its slug the two ends could never meet: the node rendered "In a deleted
    notebook" while that notebook sat live in the same row's own picker. Reproduced in a browser
    ...
    and reachable from the first `rlm-notebook ask --notebook "reading list"` or any HTTP caller.
    """
    from rlm_notebook import inbox
    ...
    for notebook_id in ("Foo Bar", "台灣研究筆記", "plain"):
        filed = client.post(f"/inbox/{node['id']}/promote", json={"notebook_id": notebook_id, "create": True})
        assert filed.status_code == 200, filed.text

    listing = {n["id"]: n for n in client.get("/notebooks").json()["notebooks"]}
    by_slug = {n["slug"]: n["id"] for n in listing.values()}
    for membership in inbox.memberships_for(node["id"]):
        assert membership.notebook_id in by_slug, (
            f"a membership names {membership.notebook_id!r}, which nothing in GET /notebooks "
            f"matches — the client can only render this as 'a deleted notebook'. "
            f"Listing slugs: {sorted(by_slug)}"
        )

    # ...and the field says what it claims: the slug of the id beside it.
    for notebook in listing.values():
        assert notebook["slug"] == slug(notebook["id"])
...
def test_a_notebook_can_be_deleted_and_its_nodes_survive(client, tmp_path):
    ...
    "Untitled notebook · 0" forever. The product ships Forget for a node and ✕ for a source, and
    `app.js` even carried the string "a deleted notebook" for a state nothing could produce.

    The nodes survive, which is the half that makes deleting safe to offer: promotion COPIES into a
    notebook (invariant 78), so a capture outlives any notebook it was filed into.
    """
    from rlm_notebook import inbox
    from rlm_notebook.notebook import notebook_path

    node = client.post("/inbox", json={"texts": ["worth keeping past the notebook"]}).json()["nodes"][0]
    _settle(client, node["id"])
    assert client.post(
        f"/inbox/{node['id']}/promote",
        json={"notebook_id": "doomed", "create": True},
    ).status_code == 200
    assert notebook_path("doomed").exists()
    assert len(inbox.memberships_for(node["id"])) == 1

    resp = client.delete("/notebooks/doomed")
    assert resp.status_code == 200, resp.text
    ...
    assert client.get(f"/inbox/{node['id']}").status_code == 200
    blocks = client.get(f"/inbox/{node['id']}/source").json()["source"]["blocks"]
    assert "".join(b["text"] for b in blocks) == "worth keeping past the notebook"

    # Deleting it again is a 404, not a second success.
    assert client.delete("/notebooks/doomed").status_code == 404


def test_deleting_a_notebook_takes_its_audio_with_it(client):
    """One file per notebook is what made retention a non-question (invariant 42). Leaving it behind
    would make it one, and `find_audio` would then serve an episode for a notebook that is gone."""
    from rlm_notebook.notebook import audio_path

    client.post("/notebooks/withaudio/sources", json={"sources": ["https://example.com/a"]})
    ...
    assert client.delete("/notebooks/withaudio").status_code == 200
    assert not episode.exists(), "the episode outlived the notebook it belongs to"


def test_a_notebook_with_a_run_in_flight_refuses_to_be_deleted(client):
    """Deleting the file under a running worker leaves it writing an answer into a notebook that no
    longer exists — and `mutate_notebook` would helpfully re-create it, a notebook resurrected by
    its own deletion. The Stop the reader already has is the one that ends the run."""
    import types

    client.post("/notebooks/busy/sources", json={"sources": ["https://example.com/a"]})
    api._ACTIVE_RUNS["busy"] = types.SimpleNamespace(process=None)
    try:
        resp = client.delete("/notebooks/busy")
        assert resp.status_code == 409, resp.text
        assert "stop it first" in resp.json()["detail"]
    finally:
        api._ACTIVE_RUNS.pop("busy", None)
    # Still there, which is the point of refusing.
    assert client.get("/notebooks/busy").status_code == 200
    assert client.delete("/notebooks/busy").status_code == 200
...
def test_the_inbox_upload_refuses_a_chunked_body_with_411():
    """Invariant 30, on the product's DEFAULT drop target — and nothing pinned it.

    `test_api.py` pins the same guard for `/notebooks/{id}/sources/upload`; deleting it from
    `/inbox/upload` left all 999 tests green, found by an independent review sending a real chunked
    ...
    with pytest.raises(api.HTTPException) as exc_info:
        asyncio.run(api.upload_into_inbox(_NoContentLength()))
...
def test_the_inbox_upload_refuses_an_oversized_declaration_before_parsing():
    with pytest.raises(api.HTTPException) as exc_info:
        asyncio.run(api.upload_into_inbox(_DeclaredOversized()))
    assert exc_info.value.status_code == 413


def test_stop_reaches_the_summary_batch_that_has_not_started_yet(client, monkeypatch):
    """**"Stop means stop, not stop-one-of-the-two"** — `cancel_inbox_intake`'s own comment, and it
    ...
    client.post("/inbox", json={"texts": ["something worth summarising"]})
    node = client.get("/inbox").json()["nodes"][0]
    _settle(client, node["id"])
    started.clear()
    ...
    client.post("/inbox", json={"texts": ["a second thing, deliberately"]})
    assert api._DISTIL["cancel"] is False, "throwing something new in must resume the pass"


def test_promoting_into_a_deleted_notebook_does_not_bring_it_back(client):
    """**A STALE PICKER OPTION was enough — no race required.**

    The Inbox's "File into…" `<select>` is built from the notebook list fetched when the page
    rendered, so an option can name a notebook that has since been deleted. Promoting through one
    re-created it: same handle, same slug, holding one source and none of its title, sources, notes,
    turns or overview, back in the facet rail. `DELETE /notebooks/{id}` refuses while a run is in
    ...
    new notebook" (invariant 37) and sends an existing id otherwise.
    """
    node = client.post("/inbox", json={"texts": ["worth filing"]}).json()["nodes"][0]
    _settle(client, node["id"])

    first = client.post(
        f"/inbox/{node['id']}/promote",
        json={"notebook_id": "nb-abc12345", "create": True},
    )
    assert first.status_code == 200, first.text
    assert client.delete("/notebooks/nb-abc12345").status_code == 200

    stale = client.post(f"/inbox/{node['id']}/promote", json={"notebook_id": "nb-abc12345"})
    assert stale.status_code == 404, (
        f"a deleted notebook came back through a stale picker option: {stale.status_code}"
    )
    assert "nb-abc12345" not in [n["id"] for n in client.get("/notebooks").json()["notebooks"]]


def test_a_note_cannot_resurrect_a_notebook_deleted_while_it_was_in_flight(client, monkeypatch):
    """The last write site still passing a constant. Lazy creation stays — a notebook really can
    start life as a note, which this endpoint's docstring promises — but a notebook deleted while
    the write was in flight must not be re-created by it."""
    import threading

    from rlm_notebook.notebook import notebook_path

    client.post("/notebooks/noted/sources", json={"texts": ["a source"]})
    ...
    worker = threading.Thread(
        target=lambda: result.update(
            resp=client.post("/notebooks/noted/notes", json={"text": "a thought"})
        ),
        daemon=True,
    )
    worker.start()
    assert writing.wait(10)
    assert client.delete("/notebooks/noted").status_code == 200
    release.set()
    worker.join(10)

    #: The notebook stays gone. The racing write's own status is deliberately NOT asserted: whoever
    #: takes the per-notebook lock first legitimately wins, so a 200 here means the note landed
    #: before the delete and a 404 means after. What must never happen is the file coming BACK.
    assert not notebook_path("noted").exists(), "a note resurrected a deleted notebook"
    assert result["resp"].status_code in (200, 404, 409), result["resp"].status_code

    # ...and a note can still START a notebook, which is the behaviour this endpoint promises.
    assert client.post("/notebooks/fresh-by-note/notes", json={"text": "hello"}).status_code == 200
    assert notebook_path("fresh-by-note").exists()
