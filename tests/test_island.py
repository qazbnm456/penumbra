...
def test_the_island_asks_the_shell_for_only_the_five_fixed_paths():
    """Invariant 81: the island's only channel to the shell is a navigation to one of five fixed,
    dataless paths. Anything else it navigates to would be a new, unreviewed request."""
    asked = set(re.findall(r'location\.href = "([^"]+)"', JS))
    verbs = ("open", "menu", "rest", "note", "peek")
    assert asked == {f"/__shell/{verb}" for verb in verbs}, asked
