...
The automatic pass leaves long captures for a press. It runs on the intake worker's thread, and an RLM run holding that thread for minutes would keep a capture dropped meanwhile at `queued`. Its per-batch cap still counts captures, and with long ones deferred each of those is one call. It does not align either, for the same reason; the names it writes are aligned at the end of the next pass the reader presses.
