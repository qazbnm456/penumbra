...
`GET /orbits` makes every orbit enumerable, and `OrbitSummary.title` is a model-written summary of each orbit's subject (invariant 37). A token holder therefore sees a one-line summary of every orbit. That fits the accepted posture, but it is more than metadata.
