...
So the allocator is a high-water mark that only rises (`schema.Orbit.source_seq`), and ids may have holes; nothing reads them as a count. It is stored rather than re-derived, because the hazard reaches outside the orbit file: `api.py` drops the Horizon's `NodeMembership` rows for a removed source on the promise that its id never comes back, and `orbit.py` cannot read that store. For an orbit saved before the field existed, the first allocation recovers a mark from the source ids the file still references. That cannot see membership rows, but it covers the damaging case without a migration. A tripwire walks the schema for `source_id` and `source_ids` fields and fails if the recovery scan misses one.
...
`remove_source` deletes the first match by index and raises on a miss. `mutate_orbit` writes the file unless the delta raises, so returning `False` meant a 404 still rewrote the file and bumped the modification time the picker sorts by (invariant 53).
