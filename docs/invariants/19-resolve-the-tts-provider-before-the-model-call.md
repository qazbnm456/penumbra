...
In the original order, a misconfigured `PN_TTS_PROVIDER` surfaced only after the transcript had been generated, wasting a real model call. Do not move `get_tts_provider(...)` back after `GeneratePodcastScript().run(...)`.
