...
Before it, `voice_map` came straight from `PN_TTS_VOICE_HOST_A` and `PN_TTS_VOICE_HOST_B`, so a correct Chinese script was read by the default English voices. That is a routing bug, not a synthesis one, and any provider would have had it.
...
Precedence: an explicitly set `PN_TTS_VOICE_HOST_A` or `PN_TTS_VOICE_HOST_B` beats the settings file (invariant 41), which beats the language default, which beats the shipped English cast. The file ranks above the language default because both it and the environment are a person choosing a voice. Explicitness is read from the raw environment, not by comparing against the default value, because an operator who deliberately picks the English default for a Chinese orbit is making a choice. The two voices resolve independently. For an unknown language, `default_voices` returns `None` and the configured voices stand; a voice in the wrong language is bad, but substituting a voice for a language nobody asked for is worse.
