...
tool the model can call. The provider is selected by `PN_TTS_PROVIDER` (default `"edge-tts"`, a
...
class TTSProvider(Protocol):
    ...
    def validate(self, language: str | None, voice_map: dict[str, str]) -> None:
        ...
        stops a typo'd `PN_TTS_PROVIDER` from wasting a model call, but an independent review found
...
#: `PN_OUTPUT_LANGUAGE` ("zh-TW"). An unknown language returns None and the configured defaults
...
def default_voices_for(language: str | None) -> tuple[str, str] | None:
    ...
    **This is the gap that kept the podcast out of the previous slice's language work**: nothing in
    ...
    `PN_TTS_VOICE_HOST_A/B` and `synthesize` spoke whatever it was handed — so a correct Chinese
...
class ChatterboxProvider:
    """`PN_TTS_PROVIDER=chatterbox` — a fully local, multilingual provider (the `chatterbox` extra).
    ...
    **Three costs, all measured on Apple Silicon rather than assumed, and none of them hidden:**
    ...
    3. It ships ONE built-in voice, so two distinguishable hosts need reference clips. See
       `penumbra/voices/README.md` for where the two shipped clips come from and why.
    """
...
def get_tts_provider(name: str) -> TTSProvider:
    """Look up a `TTSProvider` by `PN_TTS_PROVIDER` name. Raises `TTSError` (not `KeyError`) on an
