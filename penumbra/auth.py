...
- Any page open in the user's browser can issue state-changing requests to `127.0.0.1:<port>`
  without ever reading a response. `DELETE /orbits/{id}/sources/{id}` destroys a source
  irreversibly and `PUT /settings` rewrites settings for orbits the caller never named.
- DNS rebinding goes further and reaches READS: the attacker's own page keeps its origin, the name
  ...
  are same-origin. `GET /orbits/{id}/sources/{id}` returns a source's FULL TEXT, which invariant
...
- **No way to turn the token off.** A `--no-token` flag re-creates the exact hole this closes, and
  ...
  their own auth can still set `PN_API_TOKEN` to a value their proxy injects.
...
def api_token() -> str:
    ...
    `PN_API_TOKEN` wins when set — that is the channel the desktop shell will use to inject a token
    ...
    """
    return (os.getenv("PN_API_TOKEN") or "").strip() or _MINTED


def token_is_minted() -> bool:
    """True when nobody supplied a token and this process made one up, which is the only case where
    it has to be PRINTED — otherwise whoever set `PN_API_TOKEN` already knows it."""
    return not (os.getenv("PN_API_TOKEN") or "").strip()
...
def _extra_allowed_hosts() -> frozenset[str]:
    """`PN_ALLOWED_HOSTS` — the operator's explicit carve-out from the rule below.
    ...
    `PN_FETCH_ALLOW_CIDRS` and invariant 25 uses for allowing a non-loopback `--host` at all.
    """
    raw = os.getenv("PN_ALLOWED_HOSTS") or ""
...
def host_is_allowed(host_header: str | None) -> bool:
    ...
    a literal IP is a literal IP. Names go through `PN_ALLOWED_HOSTS`.
    """
    if not host_header:
        return False
