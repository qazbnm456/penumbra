...
Extracted from `orbit.save_orbit`, which grew it after an independent review found a plain
...
Deliberately its own module rather than living in `orbit.py`: `config.py` needs it too, and
`config.py` importing `orbit.py` would drag in the whole ingestion/parser chain to write one
file.
"""
