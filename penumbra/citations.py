...
def strip_markers(text: str) -> str:
    ...
    model actually produced, and rewriting it on the way in would make an old orbit and a new one
    disagree about their own history. Doing it on the way out also fixes every orbit already on
        ...
        if source is None:
            verified.append(
                VerifiedCitation(
                    citation=citation,
                    verified=False,
                    reason=f"no source with id {citation.source_id!r} in this orbit",
                )
            )
            continue
