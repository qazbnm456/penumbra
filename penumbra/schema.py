...
#: promise of the Horizon is that throwing something in always works. A node whose distillation failed
...
class Source(BaseModel):
    ...
    #: to empty, so orbits written before this field existed still load.
...
class Node(BaseModel):
    """One captured thing in the Horizon (Tier 0) — NOT yet part of any orbit.
    ...
    **A node is a parsed `Source` that is not bound to an orbit yet, plus the metadata distilled
    ...
    The blocks live on disk (`horizon/nodes/<id>.json`), not in this object and not in the index: this
    is the row a listing renders, and a listing of a thousand nodes must not carry a thousand
    corpora. `horizon.node_source` loads them when something actually needs the text.

    Distinct from `Source` on purpose. A `Source` belongs to exactly one orbit and carries an id
    that orbit assigned; a node is global, pre-curation, and may end up in zero, one or several
    orbits — which is the "different facets of yourself" premise the Horizon exists for.
    """
...
class Distillation(BaseModel):
    """What the cheap summariser produces for one Horizon node (`distill.py`).

    A separate shape from `Node` rather than four loose return values, because it is what
    `horizon.update_node` is handed as one delta (invariant 78) and what a test compares against.
...
class NodeMembership(BaseModel):
    ...
    Recorded because promotion does NOT consume the node (`horizon.promote_node`): the node is the
    thing that persists, and an orbit is a view over a selection of them. Without this row,
    "which orbits is this in?" would have to be answered by parsing every orbit file.
    """

    node_id: str
    orbit_id: str
    #: The id `orbit.append_sources` ASSIGNED — never the node id. Invariant 50: ids come from the
...
class Citation(BaseModel):
    ...
    #: still loads — the same backward-compatible precedent `ChatTurn.run_id` and `Orbit.notes`
...
class Answer(BaseModel):
    ...
    #: found useful appeared exactly once per orbit and never again.
    #:
...
class KeyInsight(BaseModel):
    ...
    takeaway across a collection's sources, in one sentence."""
...
#: three minutes regardless of what was in the orbit: the instructions asked for "a natural
#: episode length" and never named a number, and the orbit with the MOST sources produced the
#: SHORTEST of the four measured — so "let the sources decide" demonstrably was not happening.
#: (The orbits that measurement came from were cleared at the owner's request, so it is not
#: reproducible from what is on disk — and `orbits/` cannot confirm or refute it either way.)
PodcastLength = Literal["short", "default", "long"]

#: How much WALL CLOCK each tier is allowed, as a multiple of `PenumbraConfig.run_timeout_seconds`.
#:
...
#: else was ever written before `killpg`. On the same orbit and the same model, an ordinary chat
...
class ChatTurn(BaseModel):
    """One past (question, answer) exchange in an `Orbit`'s history."""
    ...
    #: link back to `traces/{run_id}.jsonl` on a re-opened orbit's history, even long after the
...
class Note(BaseModel):
    ...
    real `Source` (`orbit.promote_note`), at which point it's grounded and citable exactly like
...
class Overview(BaseModel):
    """The orbit's front page: a Summary plus the FAQ questions offered as follow-ups.

    The ONE guide artifact persisted onto an orbit (a deliberately narrow cut of the long-deferred
    ...
    #: `PN_TRACE_RETENTION_DAYS` collects it — same as `ChatTurn.run_id`, degrading to no affordance
...
class Podcast(BaseModel):
    ...
    **This deliberately reverses Phase 2's "no audio is ever persisted past one request".** That
    ...
    asking where the mp3 was. The audio is stored as ONE file per orbit, replaced on regenerate,
    so the growth is bounded by the number of orbits rather than by the number of generations.
    """
...
class Orbit(BaseModel):
    """Sources plus chat history that survive across `ask` invocations (see `orbit.py`). This is
    the whole persisted unit — one JSON file per orbit, no database."""

    id: str
    #: A human label, suggested from the sources (`naming.py`) — NOT the id. The id stays a stable
    #: handle that filenames, `ChatTurn.run_id` prefixes and every URL are built from, so an orbit
    #: can be titled (and one day retitled) without moving a file or invalidating a run id. Optional
    #: and defaulting to None, so every orbit written before this field existed still loads —
    #: the same backward-compatible precedent `ChatTurn.run_id` and `Orbit.notes` already set.
    title: str | None = None
    ...
    #: behaviour. `PN_OUTPUT_LANGUAGE` overrides it at generation time, so changing the env takes
    ...
    #: orbit file (see `orbit.audio_path`), not in here — a multi-MB base64 blob inside the
    #: JSON would be re-parsed on every single read of this orbit.
    podcast: Podcast | None = None
    #: The persisted chat overview, if one has been generated. Optional and defaulting to None, so
    #: orbits written before it existed still load (the precedent `run_id`/`notes`/`title` set).
    overview: Overview | None = None
    sources: list[Source] = Field(default_factory=list)
    turns: list[ChatTurn] = Field(default_factory=list)
    notes: list[Note] = Field(default_factory=list)
    #: The highest source number ever ALLOCATED here — not the highest currently in use. Bumped by
    #: `orbit.next_source_id`, and the reason it has to be persisted rather than re-derived is
    ...
    #: orbit written before this field existed still loads (the precedent `run_id`/`notes`/
