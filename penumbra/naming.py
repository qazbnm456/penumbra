"""Suggest a human title for an orbit from the sources already in it.

A user should not have to invent an id before they can add their first source. They did have to —
the web UI refused with "Open or name an orbit first" — which made the very first interaction
...
#: is called right after the FIRST source lands, when the whole orbit is usually one file.
_EXCERPT_CHARS = 4000
...
_INSTRUCTIONS = """\
Write a short, specific title for a research collection containing these sources.
...
- Name what the COLLECTION is about, not what one source is called. `sources` is a sample of EVERY
  source in the collection, separated by `[[SRC:...]]` markers — read past the first one. Copying or
  translating source one's own title is the failure mode here: a collection holding a paper, a lab
...
def normalize_title(raw: str) -> str:
    ...
    a GENERATED title falls back to something derived from the origins (an orbit must end up with
    ...
    # any token holder can write through `PUT /orbits/{id}/title` (invariants 25 and 53). It is
    # the same
...
Decide which language this person wants their research collection WRITTEN IN.
...
- `questions`: anything this person has actually typed. The STRONGEST signal when present, because
  it is the one place they chose a language for THIS collection rather than for the app in general.
...
class SuggestLanguage:
    """Which language to write this orbit's artifacts in — `worker.py`-compatible, like
