...
cites it — `AnswerQuestion` (`task.py`), the four Orbit Guide tasks (`guide.py`) and
...
#: same packaging reason the web assets are (`packages = ["penumbra"]`, invariant 29): a
...
_SCRIPT_NEEDLES: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    # and `PN_OUTPUT_LANGUAGE`/the settings file accept any string (`clean_language` bounds length,
...
def _wrong_script_chars(family: str) -> dict[str, str]:
    ...
    **The membership test is a legacy CODEC, not zhconv's own `SIMPONLY`/`TRADONLY` sets.** Those
    ...
    uses — measured on this project's own orbits, where all four appear in prose that is not
...
def make_grounded_validator(
    ...
    **Why not a schema validator.** Nothing rewrites a stored artifact (the strip happens on the way
    OUT), so an orbit written before this validator existed holds whatever the model produced — the
    ...
    untidy data into a corrupt-orbit 409. The check belongs
...
CITATION_RULES = """\
`sources` is a single string containing every source in this collection. Each citable block is
...
research collection is for. Everything AROUND the name still follows the language you were asked to
write in.\
"""
