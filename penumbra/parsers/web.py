...
def _fetch(url: str, *, timeout: float = 15.0) -> tuple[bytes, str]:
    ...
    **Bounded.** The read used to be `resp.read()` with no argument, on an endpoint where any token
    ...
    independence from `PenumbraConfig` (invariant 30): bytes arriving from outside in one request,
...
#: a research horizon impossible.** `trafilatura.extract` ran unconditionally, so an arXiv PDF link
