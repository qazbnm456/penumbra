...
**The signature micro-interaction is the swallow.** A captured row settles into place from just above its own position (`.node.is-new`, `@keyframes node-swallow`: `translateY(-0.5rem)`, `scaleY(0.96)` and opacity over 260ms on `cubic-bezier(0.16, 1, 0.3, 1)`, transform and opacity only). It is the only entrance animation on the surface. The only other motion is the state dot, which pulses while a node is being worked on and is otherwise still, so noticing it means something. A row opens in the details column beside the list (`selectNode`, `.node.is-selected`), as a message opens beside a mail list: the whole summary, every tag, what it talks about, its trail and its actions, while the list keeps its place. Opening it inside the row pushed every later capture down and the list jumped; beside it the Horizon uses its full width in both views, the list widening to about 38rem plus margins. A "+N" of tags is a button that opens the same column. With nothing chosen the column closes, so the list takes the width. On the map, with nothing chosen, the same column holds what is waiting (`renderHorizonTodo`, `.todo-head`, `.todo-count`): loose captures, filing suggestions and pending summaries, each a section with a count. The column closes only when that card is empty. The rings carry their own meaning (`.map-ring-label`, Newest on the inner ring and Oldest on the outer) instead of a sentence in the legend.
...
**A lens says why, not only where.** Tags (`.lens`) can be combined, as a union, with a `.lens-clear` once any is on. On the map a lens lights planets, marks the moons that carry a tag (`.map-dot.is-lensed`, larger and glowing) and fades the rest (`.is-faded`), because a lit planet alone left the reader to open each capture to find which one it meant. The card becomes the lens card: the chosen tags as removable chips (`.lens-chosen`), then each capture carrying one (`.lens-item`) with its title, four lines of its summary and the orbits it is in, so the question "what is this tag about" is answered by reading, not by clicking.

**Scanning and studying are two views of an orbit.** The map, the graph and the lens card are for scanning: what is here, what connects, what a tag is about. The columns are for studying one orbit with the AI, and the toggle calls them that (研讀, Study); the planet card's primary action goes there (Study it), and so does a lens card whose captures share one orbit (`.lens-study`, secondary beside the ask). An answer worth keeping becomes a moon with `.keep-as-moon`, marked with the orbit glyph, between Copy and the star, which is why an answer's reserved corner holds three controls.

**The knowledge graph is handled, too.** It has the map's camera (`.graph-camera`, wheel and pinch, pan, + - 0), and a node can be dragged. The pull follows links, not distance: one link away follows at 0.6 and two at 0.3, each step closing part of the gap, so the cluster trails the pointer and keeps its shape. Keeping each neighbour's distance to the dragged node was tried first and swung them all to one side into a pile. Nodes keep 28px beyond their radii apart so a label fits, and return re-runs the layout. A click on empty space lets go of what was picked, as on the map. Its details panel (`.graph-side`) has the same grip as every other side panel.
...
**Three panes, one behaviour.** The Horizon in map mode is the orbit list, the map and a details panel (`.map-panel`); an orbit in columns mode is sources, conversation and Studio. Every side panel has a `.panel-grip` (the Studio's is `.studio-resize`) that drags its width, puts it away when dragged below its collapse width, opens again only at its minimum, toggles on double-click or Enter and steps with the arrows. The middle column keeps at least 360px. Widths are remembered per panel; a put-away left panel takes no width at all, its grip riding the edge of the column beside it and showing under the pointer; a put-away right panel keeps a rail.

**Capturing is the same gesture at the top.** `.capture-dock` holds the capture field and its hint, clipped at rest to its grip under the header and sliding down near the top edge, on N, or while it holds text, focus, an error or a drag. Map and list share it, so capturing looks the same everywhere.

**The ask dock is a grip until you reach for it.** At rest it shows only `.ask-dock-grip`, a short bar that breathes, with no label, and it slides up when the pointer reaches the grip, never anywhere along the edge; the capture drawer opens the same way from its grip at the top. Open, it reads like the capture field: scope chips and a `.ask-h-add` that opens a staged picker (`.ask-picker`: tags, entities and orbits with their counts, then the names in the one chosen, Escape back up a level; typing searches every kind, and typing `/` at the start of the question opens the same picker), and the orbit composer's own box (`.ask-form`: the text on top, an `.ask-foot` row of keycaps with one word each (preview or ask, new line, commands), and one round `.ask-send`), which is the one design for writing a question everywhere. While it is open, the map's legend and summary note fade out beneath it. It stays up while it holds a running ask or an answer not yet read, because that run's Stop lives only there (invariant 47).

**The map is handled, not only watched.** One transform on `.map-world` is the camera: scroll or pinch zooms around the pointer, dragging empty space pans, and `.map-camera` holds zoom in, zoom out and return, with + - 0 and Escape from the keyboard. Clicking a planet glides the camera to it over about 600ms, eased, and holds the planet still while the camera is on it, left of centre so the card does not cover it. Every moon and every dot around the Horizon is one capture with an invisible `.map-hit` around it: pointing names it in `.starmap-tip`, clicking opens it in the card, and dragging it onto a planet files it there (a dot from around the Horizon) or moves it there (a moon from another planet), with the planets it can go to lit and a notice carrying Undo. The carry is the signature micro-interaction of the map: the dot lifts off with a shadow (`.map-carry`), trails the pointer with a little lag and stretches along its motion, leans a third of the way toward a planet it can land on, then falls in with a ripple (`.map-ripple`) or springs back with an overshoot. A hollow moon is a source not yet recorded in the Horizon (added by the CLI while the server ran; the next start records it): it can be pointed at and opens its orbit, and it cannot be carried. Clicking the Horizon (`.map-hole-hit`) lists what is filed nowhere with File into… on each, the keyboard's way to do what dragging does, and a planet's card lists its captures for the same reason.
...
- Clicking a moon marks the moon with a spreading ping and its planet only with a dashed ring, never with the planet's own halo.
...
**Nineteen kinds, after the exoplanet classes.** Besides the nine above there are the ringed giant, ice giant, hot Jupiter, ocean, eyeball (tidally locked, drawn still with its warm eye to the light), carbon (glints), sulfur (vents with halos), cloudy and red worlds, and the double planet (`.map-binary`, two `drawWorld`s turning round a barycentre while each turns back, so its lit side stays to the light). Rings (`planetRings`) are split: the far arcs drawn before the body and the near arcs after its shade, so the planet sits inside them. The deep sky (`deepSky`: `.map-nebula`, `.map-galaxy`, `.map-double`) is radial gradients, never a blur filter, because the camera transforms the world every frame and a filter would re-rasterise.
...
**Rest is the map as a wallpaper.** `.is-ambient` fades every control and every piece of text on the map (names, counts, the Horizon's line, bridge labels), pins `.starmap` over the window, and in the desktop app asks the shell for simple full screen (`shellWindow`), undone on exit; the camera drifts between wide views every 16 to 24 seconds. A key, a click, a wheel or a nudge of the pointer ends it at once; only the first 0.6 seconds of pointer movement are ignored, while the window grows to the screen under the pointer. The only words at rest are `.ambient-clock`, the time and date bottom left, and opposite it, bottom right, `.ambient-note`: a capture picked at random from those already summarised, read in two layers, as glanceable displays are. At a glance, new or old, its orbit, its age and its title; for a reader who stays, its whole summary and up to four entities it talks about as copper chips (`.ambient-note-name`) with no label, wrapping to a second line rather than running off the column. The note is a reading column of about 30 CJK characters or 60 Latin a line (`clamp(22rem, 34vw, 36rem)`), the range readability studies put at 50 to 75 characters; a summary longer than about eight lines is paged at sentence boundaries (`notePages`, width-measured with CJK counting double) with dots for the pages. Each page stays as long as it takes to read unhurried (`pageDwell`: 6 CJK characters or 4 words a second, plus a little, between 8 and 30 seconds; a slower pace held a long note for most of a minute), so a note is never pulled away mid-read. `.ambient-scrim`, a faint floor of shade, keeps both readable when a comet or nebula passes behind. The middle of the sky stays clear, as ambient screens such as Apple TV's aerial saver and Chromecast's ambient mode keep their captions to a corner. It is passive recall from the reader's own sky, read from the index with no model call. The map's own labels, and an instruction to move the pointer, stay hidden. It starts after the idle time chosen in Settings (a preference of the browser, like the interface language) and never while a run, an intake or a summary pass is going, because resting hides their Stop.
...
**She introduces herself like a game character.** A right click on her, or the context-menu key, opens `.npc` under the header: her portrait (`mascot.svg`, the moon without its tile, with her smile, lantern and little moon) in a small night sky, a copper name plate, and four lines typed out a character at a time. A click anywhere on the card, or Next, finishes the line being typed and then moves on; Escape or a click outside ends it. The typing is decoration: each whole line is in a live region, and with reduced motion lines appear at once.

**Her voice is rationed.** At most one line in her voice per screen, and only where a plain message was already shown: the empty Horizon, and filing or moving on the map. Errors, settings and anything that states a guarantee stay plain.
