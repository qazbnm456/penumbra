...
**The pen and the field.** The hover shape carries a small round pen (`.pen`) at its lower right, out of the glyph's way, and it is the only way into the note state besides the menu. The field (`.note`, `.note-input`, `.note-send`) has no placeholder, because the island draws no words: the reader has just asked for it, so an empty line with a copper caret says enough. What is typed is the one text the island shows, and it is the reader's own. A sent line falls into the ring as a mote, the same picture as a drop.
