...
@dataclass
class Corpus:
    """An ordered collection of `Source`s for one orbit session."""
    ...
    def blob(self, *, max_chars: int | None = None) -> str:
        ...
        if max_chars is not None and len(text) > max_chars:
            raise CorpusTooLargeError(
                ...
                f"(PN_MAX_CORPUS_CHARS) — remove a source or raise the cap explicitly."
            )
        return text
    ...
    def excerpt(self, max_chars: int) -> str:
        ...
        exactly: a four-source orbit titled by transliterating the first source's own paper
        ...
        worse: an orbit whose later sources are in another language would resolve the wrong one.
    ...
    def filtered(self, source_ids: list[str]) -> Corpus:
        ...
        about just these sources" subset selection an orbit UI can offer (see the design
