...
def with_injection_flags(source: Source) -> Source:
    ...
    orbit (a batch of paths/URLs, an uploaded file, pasted text) applies the SAME scan, once."""
...
def kind_for(value: str) -> str:
    ...
    Exists because the Horizon (`horizon.py`) records a node the moment it is captured, before anything
    ...
    link was not a defensible answer for a research horizon.
    """
...
def ingest_new(values: list[str], *, start_index: int, skip_origins: set[str]) -> list[Source]:
    """Ingest `values` not already in `skip_origins` (an existing orbit's source origins — see
    `orbit.existing_origins`), numbering ids from `start_index` so they never collide with a
    orbit's existing sources. Re-ingesting an already-present origin, whether across two calls
